<template>
    <section
        class="c-avatar-cutter el-message-box"
        v-on:click.self="changeShowState"
    >
        ...
        onEnter() {
            if (this.imgURL) {
                if (this.returnType === 'url') {
                    ...
                    this.$store.commit('changeImgUrl',this.$refs.$canvas.toDataURL())
                } else if (this.returnType === 'file') {
                    // this.$emit('enter', this.base64ToFile(this.$refs.$canvas.toDataURL(), 'avatar.png')) // 返回文件
                    this.$store.commit('changeImgUrl',this.base64ToFile(this.$refs.$canvas.toDataURL(), 'avatar.png'))
                }
